...
async def main():
    ...
    print(f"\n🎉 DEMO COMPLETED!")
    print(f"Total research reports generated: {len(rag_system.system_state.active_reports)}")
    print(f"Total unique documents retrieved: {len(rag_system.system_state.retrieved_documents)}")
    
    # Final ChromaDB stats
    stats = rag_system.chroma_manager.get_collection_stats()
    total_docs = sum(info.get('document_count', 0) for info in stats.values())
